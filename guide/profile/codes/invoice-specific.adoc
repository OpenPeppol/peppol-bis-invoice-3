...
| 384
| Corrected invoice
| Commercial invoice that includes revised information differing from an earlier submission of the same invoice. (Germany and Slovakia only)
| 380
| Invoice
...
| 389
| Self-billed invoice
| An invoice the invoicee is producing instead of the seller. (Germany only)
| 380
| Invoice
